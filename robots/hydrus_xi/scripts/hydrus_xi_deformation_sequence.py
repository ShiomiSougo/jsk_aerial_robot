...
修正点: 負から正への変形時のみプリロード方向を補正する例外処理を追加
"""
...
class HydrusXiDeformationSequencer:
    ...
    def _step_joint1_pretension(self):
        ...
        # --- ★ ココが修正箇所 ★ ---
        # 基本ルール：進行方向と逆にプリロードをかける
        direction = -1.0 if angle_diff >= 0 else 1.0
        
        # ユーザー指摘の例外処理：負の状態から正へ移行する時のみ、トルクを反転させる
        if self.current_q['joint1'] < 0.0 and self.target_q['joint1'] > 0.0:
            direction = 1.0 
        # ----------------------------
