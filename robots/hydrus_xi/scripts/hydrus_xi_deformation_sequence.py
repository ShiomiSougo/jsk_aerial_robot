...
修正: 変形フェーズのトルクをプリロードと同方向（= -sign(angle_diff)）に生成。
      コントローラ解放直後のモーメント符号ジャンプを解消。
...
class HydrusXiDeformationSequencer:
    ...
    def _calculate_target_moment_joint1(self):
        """Joint 1 の空力変形用モーメント計算（プリロードと同方向に駆動）"""
        ...
        if remaining_angle > ANGLE_ERROR_THRESHOLD:
            if magnitude < MIN_FRICTION_TORQUE:
                magnitude = MIN_FRICTION_TORQUE

        # 減速ゾーン内では上限を漸減（ソフトランディング）
        DECEL_ZONE = 0.05
        ...
        if magnitude > dynamic_max_torque:
            magnitude = dynamic_max_torque
